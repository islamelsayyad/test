  ...
  return (
    <div className="flex-1 overflow-auto p-6 lg:p-8">
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <Button onClick={() => { setEditingGroup(null); setFormOpen(true) }} className="h-9 px-3.5 text-sm font-medium bg-accent text-accent-foreground hover:bg-accent/90 rounded-lg shadow-sm">
      ...
      {groupsList.length === 0 ? (
            ...
            <TableBody>
                {groupsList.map((group) => (
                    <TableRow key={group.id} onClick={() => { setSelected(group); setDetailOpen(true) }} className="cursor-pointer">
                        ...
                        <TableCell>
                            <Badge variant="outline" className="text-[11px] font-medium px-1.5 py-0 rounded-full text-accent border-accent/20 bg-accent/10">
                                Active
                            </Badge>
                        </TableCell>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {groupsList.map((group) => (
                <div key={group.id} onClick={() => { setSelected(group); setDetailOpen(true) }} className="group bg-card rounded-xl p-5 border border-border hover:border-accent/30 hover:shadow-sm transition-all duration-200 cursor-pointer flex flex-col">
